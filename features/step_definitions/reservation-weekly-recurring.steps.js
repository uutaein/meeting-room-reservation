...
Then(/^([\d-]+) 예약은 생성되지 않아야 한다$/, async function (dateStr) {
  ...
  const hasRecurring = actualReservations.some(r => r.recurringTitle === "주간회의");
  assert.strictEqual(hasRecurring, false);
});

When("사용자가 반복 예약 제목 없이 주간 반복 예약을 생성한다", async function () {
  const requestBody = {
    roomId: "ROOM_1",
    recurringTitle: "",
    reservationDate: "2026-06-15",
...
Given(/^반복 예약 제목이 "([^"]+)"인 주간 반복 예약이 존재한다$/, async function (title) {
  const requestBody = {
    roomId: "ROOM_1",
...
When(/^사용자가 반복 예약 수정을 위해 제목 확인값 "([^"]+)"를 입력한다$/, async function (titleConfirm) {
...
When(/^사용자가 반복 예약 취소를 위해 제목 확인값 "([^"]+)"를 입력한다$/, async function (titleConfirm) {
